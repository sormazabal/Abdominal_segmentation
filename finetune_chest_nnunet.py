#!/usr/bin/env python3
"""
Fine-tune a trained 6-class chest nnU-Net model on hand-corrected masks.

This closes the loop this project is built around: pseudo-label with
TotalSegmentator (build_pseudo_labels.py) -> train (kaggle_train_chest_nnunet.ipynb)
-> review predictions in chest_ct_annotation.py's annotation tool -> correct
...
Adding a 7th class (e.g. a nodule label) is NOT supported by this script or by
nnU-Net's fine-tuning path in general: the trained segmentation head has a
fixed number of output channels, and resizing it requires surgically editing
the checkpoint — nnU-Net has no built-in support for that. If you need new
...
Example:
    uv run python finetune_chest_nnunet.py \\
...
from chest_classes import CHEST_LABELS
...
def find_pretrained_checkpoint(results_root: Path, dataset_name: str, configuration: str, trainer: str, fold: int) -> Path:
    ...
    if not checkpoint.exists():
        raise FileNotFoundError(
            ...
            f"(kaggle_train_chest_nnunet.ipynb), or check --trainer/--configuration."
        )
    return checkpoint
...
def build_raw_dataset(corrected_dir: Path, raw_dataset_dir: Path) -> int:
    ...
    `<case>_ct.nii.gz` and `<case>_mask.nii.gz` — the pairing chest_ct_annotation.py's
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--base-dataset-id", required=True, help="nnU-Net dataset id of the already-trained base model, e.g. 501.")
    parser.add_argument("--new-dataset-id", required=True, help="New nnU-Net dataset id for the corrected data, e.g. 502.")
...
def run(args: argparse.Namespace) -> int:
    ...
    if not (nnUNet_raw and nnUNet_preprocessed and nnUNet_results):
        raise RuntimeError(
            ...
            "(same env vars used by kaggle_train_chest_nnunet.ipynb)."
        )

    base_dataset_name = f"Dataset{args.base_dataset_id}_ChestCT"
    new_dataset_name = f"Dataset{args.new_dataset_id}_ChestCTCorrected"
    ...
    if args.dry_run:
        ...
        print(
            "Run chest_ct_annotation.py with --backend nnunet --nnunet-model-dir "
