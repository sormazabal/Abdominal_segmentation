#!/usr/bin/env python3
"""
Build an nnU-Net v2 raw dataset of 6-class chest pseudo-labels.

Runs TotalSegmentator over a directory of CT volumes, collapses its output to
the 6 chest classes in chest_classes.CHEST_LABELS (Right Lung, Left Lung,
...
finetune_chest_nnunet.py. See the project README for that loop.

Example:
    uv run python build_pseudo_labels.py \\
        --input Dataset/LIDC-IDRI-0021 \\
        --output /kaggle/working/nnUNet_raw/Dataset501_ChestCT \\
        --device cpu --fast
...
input shapes chest_ct_annotation.py accepts) — or a single case path directly.
"""
...
from chest_classes import BASE_MODEL_STRUCTURES, CHEST_LABELS, combine_anatomical_masks
from chest_ct_annotation import (
    LOGGER,
    configure_logging,
    is_nifti,
    load_ct,
...
CHEST_CLASS_NAMES = [name for name in CHEST_LABELS if name != "background"]
...
def build_one_case(
    ...
    missing = [name for name in CHEST_CLASS_NAMES if counts.get(name, 0) == 0]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", "-o", required=True, help="nnUNet_raw dataset directory to create, e.g. .../Dataset501_ChestCT.")
...
def run(args: argparse.Namespace) -> int:
    ...
    header = "case".ljust(24) + "".join(name.ljust(14) for name in CHEST_CLASS_NAMES)
    print("\nVoxel counts per class (0 = class not found in this scan):")
    print(header)
    for case_id, counts in rows:
        print(case_id.ljust(24) + "".join(str(counts.get(name, 0)).ljust(14) for name in CHEST_CLASS_NAMES))
