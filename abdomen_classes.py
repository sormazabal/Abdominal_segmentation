"""
Shared 6-class abdomen anatomy label spec.

Both `abdomen_ct_annotation.py` (TotalSegmentator-backed annotation tool) and
the nnU-Net training pipeline (`build_pseudo_labels_abdomen.py`, the Kaggle
training notebook, `finetune_abdomen_nnunet.py`) import from here, so the
class set never drifts between the two.

Note: this class set is identical to `chest_classes.py`'s (Right Lung, Left
Lung, Heart, Trachea, Aorta, Spine), per explicit confirmation against the
project's actual class-list UI rather than the anatomically-abdominal organs
(liver, kidneys, spleen, pancreas) the file names might otherwise suggest.

The label *names* below (the keys of `ABDOMEN_LABELS`) are the contract: a
trained nnU-Net model's `dataset.json["labels"]` must use exactly these keys,
because `abdomen_ct_annotation.py`'s nnunet backend reads label names straight
...
def combine_anatomical_masks(
    ...
    """Collapse a raw TotalSegmentator "total" mask into the 6 classes.

    Returns the merged mask plus a name -> label dict covering the 6 base
    classes (matching ABDOMEN_LABELS minus "background") and any requested
    ...
    output_labels: Dict[str, int] = {
        name: index for name, index in ABDOMEN_LABELS.items() if name != "background"
    }
